...
log"/home/dycsales/data/logs/delive_import_logs/e-nekomail_Receive_api.log"
# log="/home/vagrant/watir_src/Ruby/logs/delive_export_logs/e-nekomail_Receive_api.log"

echo "------------------------------------------------" 2>&1 | tee $log
echo "${srcName} is start." 2>&1 | tee $log
...
echo "${srcName} is end." 2>&1 | tee $log
echo "------------------------------------------------" 2>&1 | tee $log
